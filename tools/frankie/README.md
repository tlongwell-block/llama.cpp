...
Voice context checks reuse exact token counts for unchanged rendered text segments. This CPU memo is bounded to 1024 entries and 4 MiB of accounted text and entry storage, and cleared on a full session reset. Templates and media lengths are still evaluated for every request; model inputs, sampling and the KV cache are unchanged.

Brain sampling follows the Qwen recommendations: thinking uses temperature 1, top-p .95; non-thinking uses temperature .7, top-p .8. Both use top-k 20, min-p 0 and repetition penalty 1. HTTP presence penalty defaults to 0 for thinking and 1.5 for non-thinking; realtime retains its separately configured presence penalty (default 0). HTTP requests can override `temperature`, `top_p`, `top_k`, `min_p`, `presence_penalty`, `frequency_penalty`, `repeat_penalty` and `seed`; null values use defaults. Explicit temperature 0 selects greedy decoding, intended for deterministic comparisons rather than thinking quality. Raw `/v1/completions` uses the non-thinking sampling defaults because its prompt is not interpreted as a chat template.

Side defaults to 0. The Qwen talker/code predictor use temperature .6, top-k 50, top-p 1 and the reference implementation's repetition penalty. Speech cuts at sentence boundaries, joins short openers, and bounds chunks to 50 words. The expression head pools brain layer-16 states and applies gated speaker-vector offsets, a .9 peak ceiling and a reference-relative RMS ceiling.
